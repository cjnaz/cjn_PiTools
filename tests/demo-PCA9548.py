...
from cjn_PiTools.PCA9548        import PCA9548, build_mask_bit_map, mask_to_channel_str
...
#       Channel 4:  Both MCP23008 chips connected
#       MCP23008_IO_ADDR has 4 lower bits as outputs, 4 upper bits as inputs with weak pullups
#       MCP23008_7SEG_ADDR has all bits as outputs serving as pulldowns on common anode 7-segment display
...
# pca9548_resBd_handle.write_control_reg (PCA9548_RESBD_CH)
# pca9548_irrBd_handle.write_control_reg (PCA9548_IRRBD_CH)
...
if __name__ == '__main__':
    ...
    if check_tnum('1a'):
        def func():
            pca9548_resBd_handle.write_control_reg (0x55)
            return pca9548_resBd_handle.read_control_reg ()

        dotest ("write and read the control reg", "Pass", func)
    ...
    #-------------------------------------------------------------------------
    # Exercise build_mask_str()
    if check_tnum('2a'):
        def func():
            return build_mask_bit_map (0)

        dotest ("int value 0", "0", func)

    if check_tnum('2b'):
        def func():
            return build_mask_bit_map (0x55)

        dotest ("int value 0x55", "85 (decimal)", func)
    ...
    if check_tnum('2d'):
        def func():
            return build_mask_bit_map ('7')

        dotest ("str '7'", "128 (decimal) - channel 7 enabled", func)

    if check_tnum('2e'):
        def func():
            return build_mask_bit_map ('-1')

        dotest ("str -1", "0 - No channel enabled", func)


    #-------------------------------------------------------------------------
    # Exercise mask_to_channel_str()
    if check_tnum('3a'):
        dotest ("mask selects ch 1", "0", mask_to_channel_str, 1)

    if check_tnum('3b'):
        dotest ("mask 0x55", "0, 2, 4, 6", mask_to_channel_str, 0x55)

    if check_tnum('3c'):
        dotest ("mask 0xff", "0, 1, 2, 3, 4, 5, 6, 7", mask_to_channel_str, 0xff)

    if check_tnum('3d'):
        dotest ("mask 0xff", "", mask_to_channel_str, 0)
